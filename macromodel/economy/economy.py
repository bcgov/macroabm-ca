    ...
    def record_global_trade(
        ...
        for rec_country in self.all_country_names:
            ...
            self.ts.dicts["exports_before_taxes_to_" + rec_country].append(
                np.array(
                    [
                        np.nansum(firms.ts.current("nominal_amount_sold_in_lcu_to_" + rec_country)[firm_industries == g])
                        for g in range(self.n_industries)
                    ]
                )
            )
            exports_before_taxes = np.nansum([exports_before_taxes, self.ts.current("exports_before_taxes_to_" + rec_country)], axis=0)
        ...
        for sell_country in self.all_country_names:
            ...
            self.ts.dicts["imports_from_" + sell_country].append(
                np.nansum(firms.ts.current("nominal_amount_spent_in_lcu_to_" + sell_country), axis=0)
                + np.nansum(households.ts.current("nominal_amount_spent_in_lcu_to_" + sell_country), axis=0)
                + np.nansum(government_entities.ts.current("nominal_amount_spent_in_lcu_to_" + sell_country), axis=0)
            )
            imports = np.nansum([imports, self.ts.current("imports_from_" + sell_country)], axis=0)
